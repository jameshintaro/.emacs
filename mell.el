...
;; ------------------------------------------------------------
;; XEmacs 縺ｨ FSF Emacs 縺ｮ蟾ｮ逡ｰ繧貞精蜿
;; ------------------------------------------------------------
...
;; elmo-uniq-list (from wanderlust) 繧医ｊ.
...
(defun mell-time-get-interval (time1 time2)
  (if (or (> (- (nth 0 time1) (nth 0 time2)) 0)
      ...
      1000000000 ;; 譯√≠縺ｵ繧後∈縺ｮ縺�ｓ縺｡縺榊ｯｾ蜃ｦ
...
(defun mell-color-get-cursor ()
  (if (featurep 'xemacs)
      (face-background-name 'text-cursor) ;; Emacs 縺縺ｨ void
    (cdr (assoc 'cursor-color (frame-parameters)))
    ))

(defun mell-color-get-background ()
  (if (featurep 'xemacs)
      (face-background-name 'default) ;; Emacs 縺縺ｨ nil
